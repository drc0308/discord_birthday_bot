...
from .bday_cmd_parser import parse_message_bday_add_other, parse_message_bday_add
...
def insert_bday(bday : BirthdayEntry):
    ...
    cur.execute(insert_cmd,[str(bday.user_id), str(bday.date), str(bday.username)])
    db_con.commit()

def delete_bday(bday : BirthdayEntry):
    cur = db_con.cursor()
    delete_cmd = 'DELETE from birthday WHERE user = ?'
    cur.execute(delete_cmd,[bday.user_id])
    db_con.commit()
...
async def handle_bday_add(ctx):
    res = parse_message_bday_add(ctx.message.content, '$bday-add', ctx.message.author.id, ctx.message.author)
    if res is None:
...
async def handle_bday_add_other(ctx):
    res = parse_message_bday_add_other(ctx.message.content, '$bday-add-other')
    if res is None:
        logging.info(f"Invalid message format: {ctx.message.content}")
...
async def handle_bday_delete(ctx):
    bday = BirthdayEntry()
    bday.user_id = str(ctx.message.author.id)
    delete_bday(bday)
